...
class ASTCC_Data_Visualization
{
    ...
    public function display_data_graph($atts)
    {
        ...
        wp_enqueue_script(
            "data-visualization",
            plugin_dir_url(__FILE__) . "js/data-visualization.js",
            ["jquery", "chart-js"],
            "1.0",
            true
        );
        wp_localize_script("data-visualization", "dataVisualizationAjax", [
            "ajax_url" => admin_url("admin-ajax.php"),
            "nonce" => wp_create_nonce("data_visualization_nonce"),
            "exercises" => $exercises,
        ]);

        $output = '<style>
                    .result-history-button-container {
                        text-align: center;
                        margin-bottom: 20px;
                    }
                    #show-result-history {
                    ...
                    #show-result-history:hover {
                        background-color: darkred;
                    }
                </style>';
        $output .=
            '<button id="show-result-history">לחץ להיסטוריית תוצאות</button>';
        $output .= '<div id="chart-container" style="display:none;">';
        $output .=
    ...
    {
        check_ajax_referer("data_visualization_nonce", "nonce");

        $exercises = isset($_GET["exercises"]) ? $_GET["exercises"] : [];
        ...
        foreach ($data as $exercise_name => $exercise_results) {
            ...
            foreach ($exercise_results as $result) {
                ...
                if ($result_time >= $twelve_weeks_ago) {
                    ...
                    $x_value =
                        intval($week_number) + (($day_of_week + 1) / 7 - 1 / 7);
        ...
        wp_send_json([
            "success" => true,
            "data" => [
                "regular_chart_data" => $regular_chart_data,
