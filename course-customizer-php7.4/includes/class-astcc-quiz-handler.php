...
class ASTCC_Quiz_Handler{
    ...
    public function __construct(ASTCC_Utilities $utilities = null, ASTCC_Database_Manager $database_manager = null) {
        ...
        $this->expression_evaluator = new ASTCC_Expression_Evaluator($this);
    }
    ...
    public function handle_quiz_submission($quiz_data, $user){
        ...
        add_filter('learndash_quiz_content', [$this->expression_evaluator, 'eval_on_page_expressions'], $priority, 1);
    ...
    public function handle_most_recent_quiz_completion()
    {
        ...
        error_log("handling most recent quiz completion");

        // Get the most recent statistic_ref_id for the current user
        $five_seconds_ago = time() - 5;
        ...
        if (!$statistic_ref_id) {
            error_log("No quiz completions have occurred in the last 5 seconds ");
            return "Please refresh the page or press 'Restart the quiz ".$current_time;
        }

        $invalid_answers = $this->check_for_invalid_answers($statistic_ref_id);
        error_log("Invalid answer = ". $invalid_answers);
        if ($invalid_answers > 0) {
            $this->remove_invalid_quiz_answers($statistic_ref_id);
            return "Metric Assessment completed un-successfully. Some entries were invalid, none recorded. Please retake the assessment.".$current_time;

        } 
        return "Metric Assessment completed successfully. All entries were found valid and recorded by the system.".$current_time;
    }
