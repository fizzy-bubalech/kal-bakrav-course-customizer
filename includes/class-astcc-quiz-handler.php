...
class ASTCC_Quiz_Handler
{
    ...
    public function exercise_from_question_id($question_id)
    {
        if (!isset($this->exercise_cache[$question_id])) {
            ...
            if (!$match && empty($matches[3])) {
                ...
                error_log("Line 92: " . print_r($matches, true));
            }

            if ($match) {
                error_log("Matches on line 95: " . print_r($matches, true));
                if (!empty($matches[3])) {
                    $expression = trim($matches[3][0]);
                } else {
                    $expression = trim($matches[2][0]);
                }
                error_log("LINE 97" . gettype($expression));
                $decoded_expression = html_entity_decode(
                    $expression,
                    ENT_QUOTES,
                    "UTF-8"
                );
                error_log("line 103" . $decoded_expression);
                $variables = $this->expression_evaluator->extract_variables(
                    $decoded_expression
                );
                error_log("line 107: " . print_r($variables, true));
                if (count($variables) !== 0) {
                    foreach ($variables as $variable) {
                        $variable = preg_replace('/_dbtype$/', "", $variable);
                        $this->exercise_cache[$question_id] = $this->database_manager->get_exercise_by_name(
                            $variable
                        );
                        error_log("Line 114: " . print_r($this->exercise_cache, true));
                    }
                }
            }
        }
    ...
    public function exercise_from_question_post($question_post_id)
    {
        if (!isset($this->exercise_cache[$question_post_id])) {
            ...
            $this->exercise_cache[$question_post_id] = $this->database_manager->get_exercise_by_name($variable);
        }
    ...
    private function get_question($question_id)
    {
        if (!isset($this->question_cache[$question_id])) {
            $this->question_cache[$question_id] = $this->database_manager->get_question_from_id($question_id);
        }
